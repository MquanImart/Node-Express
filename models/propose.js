...
class Propose {
    static async getPropose(id_user){
        ...
        let sql = `SELECT book.*, genre.genre_name FROM book JOIN genre ON book.genre_id = genre.id ORDER BY post_date DESC LIMIT 20;`
        ...
        if (genre.length > 0){
            sql = sql + `) ORDER BY post_date DESC LIMIT 20;`
        }
        console.log(sql_genre);
